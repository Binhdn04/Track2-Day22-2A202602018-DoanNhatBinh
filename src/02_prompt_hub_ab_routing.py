...
PROMPT_V1_NAME = "binh-rag-prompt-v1"
PROMPT_V2_NAME = "binh-rag-prompt-v2"
...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(
            PROMPT_V1_NAME,
            object=PROMPT_V1,
            description="V1 – RAG prompt ngắn gọn, trả lời 2-4 câu",
        )
    ...
    try:
        url = client.push_prompt(
            PROMPT_V2_NAME,
            object=PROMPT_V2,
            description="V2 – RAG prompt có cấu trúc, expert tone, trả lời 3-5 câu",
        )
...
def pull_prompts_from_hub(client: Client) -> dict:
    ...
    Trả về:
        {
            PROMPT_V1_NAME: ChatPromptTemplate,
            PROMPT_V2_NAME: ChatPromptTemplate,
        }
    """
    ...
    try:
        ...
        print(
            f"ℹ️  Dùng local fallback cho '{PROMPT_V1_NAME}' "
            f"(lỗi: {e})"
        )
    ...
    try:
        ...
        print(
            f"ℹ️  Dùng local fallback cho '{PROMPT_V2_NAME}' "
            f"(lỗi: {e})"
        )
...
def get_prompt_version(request_id: str) -> str:
    ...
    Quy tắc:
        hash chẵn → PROMPT_V1_NAME
        hash lẻ   → PROMPT_V2_NAME

    Cùng request_id luôn cho cùng kết quả.
    """
...
def ask_ab(retriever, llm, prompt, question: str, version: str) -> dict:
    ...
    Bước:
      ...
      c) Chạy prompt → llm → output parser
      d) Trả về question, answer và version
    """
...
def main():
    ...
    # Push prompts lên Hub
    push_prompts_to_hub(client)

    # Pull prompts lại từ Hub
    ...
    print(
        "✅ Bước 2 hoàn thành! "
        "Kiểm tra Prompt Hub và traces trên LangSmith."
    )
